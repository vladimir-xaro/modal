...
interface I_ModalConstructorConfig {
	...
	el:		HTMLElement;	// main wrapper
	visible?:	boolean;	// show after init
	animations?:	boolean;	// OR transtions (priority, default)
	transitions?:	boolean;	// OR animations
	attr?: {
		close?:			string;	// for add close listeners
		target?:		string;	// open modal by
		id?:			string;	// unique modal id
	};
	allow?: {
		closeEsc?:		boolean;	// allow close modal by esc key
		closeAttr?:		boolean;	// allow close modal by [attr.close]
		animateContainer?:	boolean;	// modal will use css animations on fade in/out
		animateBackdrop?:	boolean;	// modal will use css transitions on fade in/out
	};
	selector?: {
